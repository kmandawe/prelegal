import type { MndaFormValues } from "./schema";
import { formatDate } from "./utils";

export const MNDA_STANDARD_TERMS: Array<{ heading: string; body: string }> = [
  {
...
export type DerivedFields = {
  ...
  termOfConfidentialityLabel: string;
  governingLaw: string;
  jurisdiction: string;
  modifications: string;
};

export function deriveFields(values: MndaFormValues): DerivedFields {
  return {
    purpose: values.purpose || "[Purpose]",
    effectiveDateLabel: values.effectiveDate
      ? formatDate(values.effectiveDate)
      : "[Effective Date]",
    mndaTermLabel:
      values.mndaTermKind === "untilTerminated"
        ? "Continues until terminated in accordance with the terms of the MNDA."
        : `Expires ${values.mndaTermYears ?? "[N]"} year(s) from Effective Date.`,
    termOfConfidentialityLabel:
      values.termOfConfidentialityKind === "perpetuity"
        ? "In perpetuity."
        : `${values.termOfConfidentialityYears ?? "[N]"} year(s) from Effective Date, but in the case of trade secrets until Confidential Information is no longer considered a trade secret under applicable laws.`,
    governingLaw: values.governingLawState || "[Governing Law]",
    jurisdiction: values.jurisdiction || "[Jurisdiction]",
    modifications: values.modifications || "None.",
  };
}

export function fillStandardTermBody(
  body: string,
  derived: DerivedFields,
): string {
  return body
    .replaceAll("{GoverningLaw}", derived.governingLaw)
    .replaceAll("{Jurisdiction}", derived.jurisdiction);
}
